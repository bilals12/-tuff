...
class aWAF {
    ...
    function getArray($Type) {
        switch ($Type) {
            case 'SQL':
                return array("'", '´', '..', ';', 'SELECT FROM', 'SELECT * FROM', 'FROM', 'VALUES', 'DELETE FROM', 'ONION', 'union', 'UNION', 'DROP TABLE', '--', 'INSERT INTO', 'UPDATE `users` SET', 'UPDATE settings SET', 'UPDATE `settings` SET', 'UPDATE users SET', 'WHERE username', ') or true--', ') or ("")=(', '" or true--', '" or ""="', '" or 1--', '" or "x"="', 'WHERE id', 'DROP TABLE', '0x50', 'mid((select', 'union(((((((', 'concat(0x', 'concat(', 'OR boolean', 'or HAVING', "OR '1", '0x3c62723e3c62723e3c62723e', '0x3c696d67207372633d22', '+#1q%0AuNiOn all#qa%0A#%0AsEleCt', 'unhex(hex(Concat(', 'Table_schema,0x3e,', '0x00', // \0  [This is a zero, not the letter O]
                '0x08', // \b
                '0x09', // \t
                '0x0a', // \n
                '0x0d', // \r
                ...
                '0x5c', // \\
                '0x5f', // \_
                '%2F', 'exec', 'chmod', 'chown', 'eval', 'shell_exec', 'curl_multi_exec', 'apache_setenv', '..');
            break;
            case 'XSS':
                return array('<img', 'img>', '<image', 'document.cookie', 'onerror()', 'script>', '<script', 'alert(', "curl", "telnet", "bash", "|", ";", "ncat", "netcat", "ping", "passwd", 'window.', 'String.fromCharCode(', 'javascript:', 'onmouseover="', '<BODY onload', '<style', 'svg onload');
            break;
            default:
                return false;
            break;
