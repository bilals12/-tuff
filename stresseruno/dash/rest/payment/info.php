...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        echo json_encode(array('status' => 'success', 'message' => 'Processing..', 'id' => $payment['id'], 'coin' => $payment['gateway'], 'address' => $payment['crypto_address'], 'amount' => $payment['crypto_amount'], 'qr' => '' . $qr . '', 'amount_paid' => $payment['amount_paid'], 'expires' => $expires, 'pstatus' => $status, 'confirmations' => '' . $payment['confirmations'] . ' of 2', 'hash' => $payment['hash']));
    }
}
?>