...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $coins = array('BITCOIN', 'ETHEREUM', 'LITECOIN', 'USDT');
    ...
    if ($amount < 2) {
        ...
        echo json_encode(array('status' => 'error', 'message' => 'Invalid amount value, minimum value is 7$'));
        die();
    }
    if ($amount > 2000) {
        $errors[] = "Invalid amount value!";
    ...
    if (empty($errors)) {
        $params = ['title' => "Deposit funds", 'currency' => "USD", 'return_url' => "https://stresser.uno/dash/deposit", 'webhook' => "https://stresser.uno/dash/rest/webhooks/sellixhook", 'email' => $user['email'], 'value' => $amount, 'gateway' => $coin, 'confirmations' => 3, 'white_label' => true];
        ...
        echo json_encode(array('status' => 'success', 'message' => 'Payment successfully generated, processing...', 'id' => $payment['id'], 'coin' => $res['data']['invoice']['gateway'], 'address' => $res['data']['invoice']['crypto_address'], 'amount' => $res['data']['invoice']['crypto_amount'], 'qr' => '' . $qr . '', 'amount_paid' => $res['data']['invoice']['crypto_received'], 'expires' => $expires, 'pstatus' => $res['data']['invoice']['status'], 'confirmations' => '' . $payment['confirmations'] . ' of ' . $res['data']['invoice']['crypto_confirmations_needed'] . '', 'hash' => $payment['hash']));
    }
}
?>