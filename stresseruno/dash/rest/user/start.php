...
if ($type == 'l4') {
    ...
    $httpversion = $user->CheckInput($_POST['httpversion']);
    $referrer = $user->CheckInput($_POST['referrer']);
    $cookies = $user->CheckInput($_POST['cookies']);
    $geoloc = $user->CheckInput($_POST['geoloc']);
    if ($user->SecureText($method) || $user->SecureText($reqmethod) || $user->SecureText($geoloc) || $user->SecureText($cookies)) {
        header('HTTP/1.0 400 Bad Request');
        exit();
    }
    ...
    if (empty($errors)) {
        ...
        foreach ($handlers as $handler) {
            ...
            while ($api = $SQLSelectAPI->fetch(PDO::FETCH_ASSOC)) {
                $FindInApi = array('[host]', '[port]', '[time]', '[method]');
                $ApiReplace = array($info['target'], $info['port'], $info['time']);
