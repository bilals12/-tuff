...
if ($type == 'l4') {
    ...
    if ($countsheduled >= '100') {
        $errors[] = "You cannot schedule more then 20 attacks!";
        echo json_encode(array('status' => 'error', 'message' => 'You cannot schedule more then 20 attacks!'));
        die();
    }
    ...
    if (($runningattacks + $countsheduled + 30) > $totalconcs) {
    ...
    if (empty($host) || empty($time) || empty($port) || empty($method) || empty($_POST['datetime']) || $port < 0 || $port > 65535) {
    ...
    $asn = $res['org'];
    if ($method == 'OVH' && $asn != 'AS16276 OVH SAS') {
        $errors[] = "You can only attack OVH servers with this method.";
        echo json_encode(array('status' => 'error', 'message' => 'You can only attack OVH servers with this method.'));
        die();
    }
