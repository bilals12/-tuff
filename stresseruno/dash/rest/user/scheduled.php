...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    while ($attack = $SelectAttacks->fetch(PDO::FETCH_ASSOC)) {
        ...
        $data[] = array("id" => $attack['id'], "target" => '' . $attack['target'] . ':' . $attack['port'] . '', "method" => $attackmethod, "created" => date('m-d-Y H:i:s', strtotime($attack['created'])), "scheduled" => date('m-d-Y H:i:s', $attack['datetime']), "action" => '<button type="button" id="deleteschbtn" onclick="DeleteSchedule(' . $attack['id'] . ')" class="btn btn-danger btn-sm"><span id="deletesch_def"><i class="fa-solid fa-trash-can"></i></span><span id="deletesch_loadi" style="display: none;"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span></span></button>');
    }
