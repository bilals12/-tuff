...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    while ($ticket = $GetTickets->fetch(PDO::FETCH_ASSOC)) {
        ...
        $data[] = array("subject" => '<a onclick="ViewTicket(' . $ticket['id'] . ')">#' . $ticket['id'] . ' ' . $ticket['subject'] . '</a>', "status" => '<span class="vnm-dark-badge py-1 px-2 rounded-lg">' . ucfirst($ticket['status']) . '</span>', "created" => $created,);
    }
