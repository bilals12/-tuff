...
function validate_order($order_uniqid) {
    $sellixapikey = '10ziTv6PED5ifLl6FuuoF6jBqKi2rcEoArosiL6CSGWgrmWH6JmHitqTzCxkgTU9';
...
if ($valid_order) {
    if ($status == 'PARTIAL') {
        ...
        $SQLUpdate->execute(array(":uniqid" => $valid_order['uniqid']));
        $SQLUpdatesec2 = $odb->prepare("UPDATE `users` SET `balance` = `balance` + :bal WHERE `email` = :email");
        $SQLUpdatesec2->execute(array(':email' => $valid_order['customer_email'], ':bal' => $valid_order['total']));
    }
}
?>