...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    function generateRandom($digits) {
        ...
        $rand_str = '';
        while (strlen($rand_str) < $digits) $rand_str.= substr(str_shuffle($charset), 0, 1);
        return $rand_str;
    }
    if (empty($errors)) {
        ...
        echo json_encode(array('status' => 'success', 'message' => 'Authorization key successfully generated.'));
        die();
    }
} else {
    header('HTTP/1.0 404 Not Found');
